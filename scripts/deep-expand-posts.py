...
def generate_long_content(title):
    intro = INTRO_TEMPLATES[0].replace('{title}', title).strip()
    conclusion = CONCLUSION_TEMPLATES[0].replace('{title}', title).strip()
    
    # Shuffle sections so each post is unique but covers all topics
    shuffled_sections = list(SECTIONS)
    random.shuffle(shuffled_sections)
    
    shuffled_code = list(CODE_BLOCKS)
    random.shuffle(shuffled_code)
    
    body_parts = [intro]
    for i, section in enumerate(shuffled_sections):
        body_parts.append(section.strip())
        if i < len(shuffled_code):
            body_parts.append(shuffled_code[i].strip())
...
def process_file(filepath):
    ...
    new_body = generate_long_content(title)
    ...
    print(f"Updated {os.path.basename(filepath)} | New Word Count: {new_word_count}")
    return True
...
def main():
    ...
    print(f"\nSuccessfully expanded {updated} posts to >800 words with dense architectural content.")
